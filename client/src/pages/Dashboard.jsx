import { FilePenLineIcon, Form, LoaderCircle, PencilIcon, Plus, TrashIcon, UploadCloudIcon, XIcon } from 'lucide-react'
...
const Dashboard = () => {
  ...
  const uploadResume = async (event) =>{
    ...
    try {
      ...
      toast.error("Unable to extract text from this pdf")
      console.log(error)
            ...
            showCreateResume && (
              <form onSubmit={createResume} onClick={()=> setShowCreateResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
                <div onClick={e=> e.stopPropagation()} className='relative bg-slate-50 border shadow-md rounded-lg w-full max-w-sm p-6'>
                  ...
                  <input onChange={e => setTitle(e.target.value)} value={title} type="text" placeholder='Enter resume title' className='w-full px-4 py-2 mb-4 focus:border-[#a118bdfe] ring-[#a118bdfe]' required/>

                <button className='w-full py-2 bg-[#a118bdfe] text-white rounded hover:bg-[#650f5e] transition-colors'>Create Resume</button>
...

          {
              ...
              <form onSubmit={uploadResume} onClick={()=> setShowUploadResume(false)} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
                ...
                <div>
                  <label className='block text-sm text-slate-700' htmlFor="resume-input">
                    Select resume File 
                    <div className='flex flex-col items-center justifu-center gap-2 border group text-slate-400 border-slate-400 border-dashed rounded-md p-4 py-10 my-4 hover:border-purple-500 hover:text-purple-700 cursor-pointer transition-colors'>
                ...
                <button disabled={isLoading} className='w-full py-2 bg-[#a118bdfe] text-white rounded hover:bg-[#650f5e] transition-colors flex flex-row items-center justify-center '>
                   {isLoading ? 'Uploading...' : 'Upload Resume'}
                  {isLoading && <LoaderCircle className='ml-2 animate-spin size-4 text-white'/>}
            ...
            editResumeId && (
              <form onSubmit={editTitle} onClick={()=> setEditResumeId('')} className='fixed inset-0 bg-black/70 backdrop-blur bg-opacity-50 z-10 flex items-center justify-center'>
                ...
                <button className='w-full py-2 bg-[#a118bdfe] text-white rounded hover:bg-[#650f5e] transition-colors'>Update</button>
